 ...
 * *
 * If -DSENSEI_DISABLE_LOGGING is passed as a compiler argument, all logging code
 * disappears without a trace. Useful for testing and outside releases.
 *
 * Usage:
 * Call SENSEI_GET_LOGGER before any code in each file or preferably call
 * SENSEI_GET_LOGGER_WITH_MODULE_NAME() to set a module name that will be
 ...
 */
enum SENSEI_LOG_ERROR_CODE
{
    SENSEI_LOG_ERROR_CODE_OK = 0,
    SENSEI_LOG_ERROR_CODE_INVALID_LOG_LEVEL = 1,
    SENSEI_LOG_FAILED_TO_START_LOGGER = 2,
    SENSEI_LOG_ERROR_CODE_INVALID_FLUSH_INTERVAL = 3
};
...
#else
#define SENSEI_LOG_DEBUG_IF(condition, msg, ...)    if (condition) { elk::Logger::logger_instance->debug(condition, "{}" msg, local_log_prefix, ##__VA_ARGS__);}
#endif
#define SENSEI_LOG_INFO_IF(condition, msg, ...)     if (condition) { elk::Logger::logger_instance->info("{}" msg, local_log_prefix, ##__VA_ARGS__); }
 ...
 */
#define SENSEI_INITIALIZE_LOGGER(FILE_NAME, LOGGER_NAME, MIN_LOG_LEVEL, ENABLE_FLUSH_INTERVAL, LOG_FLUSH_INTERVAL) \
    elk::Logger::init_logger(FILE_NAME, LOGGER_NAME, MIN_LOG_LEVEL, ENABLE_FLUSH_INTERVAL, LOG_FLUSH_INTERVAL)


#define SENSEI_LOG_GET_ERROR_MESSAGE(retcode) \
...
public:
     ...
     *        any object that use SENSEI_GET_LOGGER, e.g. at beginning of main.
     *
     ...
     */
    static SENSEI_LOG_ERROR_CODE init_logger(const std::string& file_name,
     ...
     */
    static std::string get_error_message(SENSEI_LOG_ERROR_CODE status);
...
#define SENSEI_INITIALIZE_LOGGER(FILE_NAME, LOGGER_NAME, MIN_LOG_LEVEL) 0
#define SENSEI_LOG_GET_ERROR_MESSAGE(retcode) ""
