...
protected:
    ...
    void SetUp()
    {
        ...
        config_cmds.push_back(CMD_UPTR(factory.make_set_enabled_command(_sensor_idx, _enabled)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_sending_mode_command(_sensor_idx, _sending_mode)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_invert_enabled_command(_sensor_idx, _inverted)));

        for (auto& cmd : config_cmds)
        {
            auto status = _mapper.apply_command(std::move(cmd));
...
TEST_F(TestDigitalSensorMapper, test_config_fail)
{
    ...
    auto ret = _mapper.apply_command(CMD_UPTR(factory.make_set_adc_bit_resolution_command(_sensor_idx, 12)));
    ASSERT_EQ(CommandErrorCode::UNHANDLED_COMMAND_FOR_SENSOR_TYPE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_slider_mode_enabled_command(_sensor_idx, false)));
    ASSERT_EQ(CommandErrorCode::UNHANDLED_COMMAND_FOR_SENSOR_TYPE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_lowpass_cutoff_command(_sensor_idx, 1000.0f)));
    ASSERT_EQ(CommandErrorCode::UNHANDLED_COMMAND_FOR_SENSOR_TYPE, ret);
}
...
TEST_F(TestDigitalSensorMapper, test_process)
{
    ...
    MessageFactory factory;
    auto ret = _mapper.apply_command(CMD_UPTR(factory.make_set_invert_enabled_command(_sensor_idx, false)));
    ...
    _mapper.process(std::move(input_val), &_backend);
    ASSERT_EQ(0.0f, _backend._last_output_value);

    input_msg = factory.make_digital_value(_sensor_idx, true);
    input_val = static_unique_ptr_cast<Value, BaseMessage>(std::move(input_msg));
    _mapper.process(std::move(input_val), &_backend);
...
TEST_F(TestDigitalSensorMapper, test_invert)
{
    // Reset relevant configuration
    MessageFactory factory;
    auto ret = _mapper.apply_command(CMD_UPTR(factory.make_set_invert_enabled_command(_sensor_idx, true)));
    ASSERT_EQ(CommandErrorCode::OK, ret);
...
TEST_F(TestDigitalSensorMapper, test_disabled_process_dont_send_values)
{
    // Reset relevant configuration
    MessageFactory factory;
    auto ret = _mapper.apply_command(CMD_UPTR(factory.make_set_enabled_command(_sensor_idx, false)));
    ASSERT_EQ(CommandErrorCode::OK, ret);
...
protected:
    ...
    void SetUp()
    {
        ...
        config_cmds.push_back(CMD_UPTR(factory.make_set_sending_mode_command(_sensor_idx, _sending_mode)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_invert_enabled_command(_sensor_idx, _inverted)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_sending_delta_ticks_command(_sensor_idx, _delta_ticks)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_adc_bit_resolution_command(_sensor_idx, _adc_bit_resolution)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_lowpass_filter_order_command(_sensor_idx, _lowpass_filter_order)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_lowpass_cutoff_command(_sensor_idx, _lowpass_cutoff)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_slider_mode_enabled_command(_sensor_idx, _slider_mode_enabled)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_slider_threshold_command(_sensor_idx, _slider_threshold)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_input_scale_range_low_command(_sensor_idx,
                                                                                                _input_scale_low)));
        config_cmds.push_back(CMD_UPTR(factory.make_set_input_scale_range_high_command(_sensor_idx,
                                                                                                 _input_scale_high)));
...
TEST_F(TestAnalogSensorMapper, test_config_fail)
{
    ...
    auto ret = _mapper.apply_command(CMD_UPTR(factory.make_set_sampling_rate_command(_sensor_idx, 1000.0f)));
    ASSERT_EQ(CommandErrorCode::UNHANDLED_COMMAND_FOR_SENSOR_TYPE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_adc_bit_resolution_command(_sensor_idx, -10)));
    ASSERT_EQ(CommandErrorCode::INVALID_VALUE, ret);
    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_adc_bit_resolution_command(_sensor_idx, 100)));
    ASSERT_EQ(CommandErrorCode::INVALID_VALUE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_sending_delta_ticks_command(_sensor_idx, -23)));
    ASSERT_EQ(CommandErrorCode::INVALID_VALUE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_lowpass_filter_order_command(_sensor_idx, 0)));
    ASSERT_EQ(CommandErrorCode::INVALID_VALUE, ret);
    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_lowpass_filter_order_command(_sensor_idx, 100)));
    ASSERT_EQ(CommandErrorCode::INVALID_VALUE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_slider_threshold_command(_sensor_idx, -20)));
    ASSERT_EQ(CommandErrorCode::INVALID_VALUE, ret);
    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_slider_threshold_command(_sensor_idx, 123456)));
    ASSERT_EQ(CommandErrorCode::INVALID_VALUE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_input_scale_range_low_command(_sensor_idx, -20)));
    ASSERT_EQ(CommandErrorCode::INVALID_RANGE, ret);
    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_input_scale_range_high_command(_sensor_idx, -20)));
    ASSERT_EQ(CommandErrorCode::INVALID_RANGE, ret);

    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_input_scale_range_low_command(_sensor_idx,
                                                                                       _input_scale_high + 1)));
    ASSERT_EQ(CommandErrorCode::CLIP_WARNING, ret);
    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_input_scale_range_high_command(_sensor_idx,
                                                                                        _input_scale_low - 1)));
    ASSERT_EQ(CommandErrorCode::CLIP_WARNING, ret);
}
...
TEST_F(TestAnalogSensorMapper, test_invert)
{
    ...
    ret = _mapper.apply_command(CMD_UPTR(factory.make_set_invert_enabled_command(_sensor_idx, true)));
    ASSERT_EQ(CommandErrorCode::OK, ret);
